...
- 生产模板的 CMS 页面绑定、preview 模板、资源和原门户源码位置；
...
- 页面生成方式：模板重新渲染、静态脚手架加数据注入，或其他明确的数据消费方式；不能仅从数据库表结构推断页面模型。
...
- `Build(Production, ...)` 自行创建数据源，并通过 runtime close 函数关闭资源；使用 Portal CMS 时从数据库读取生产模板。
...
- 发布关系、草稿和未审核排除有测试覆盖。
- 生产模板通过明确的 `template.code` 或唯一名称绑定读取 `template.source_code`；不得依赖已删除的 `page` 表，也不得把本地文件作为静默回退。
- 每次生成操作读取一致的模板快照，后台模板修改应在下一次操作生效。
- 启动和生成都校验缺失绑定、禁用记录、类型不符、选择歧义、空源码、大小、模板语法和路由安全。
...
- 与当前示例配置和生成器实现保持同步。
- 没有接入动态数据的页面区域必须明确写为“固定静态内容”，不得要求运维创建无效栏目。
...
- Portal CMS 生产模板来自数据库，原模板源工程保持只读；
- 全量测试和真实数据验收通过；
- 部署、配置、运维和回滚信息已补充到文档。
