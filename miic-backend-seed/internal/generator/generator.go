...
func New(cfg config.Config, source Source, logger *slog.Logger) (*Generator, error) {
	...
	return &Generator{cfg: cfg, source: source, logger: logger, location: location, stale: stale, sanitizer: policy}, nil
}
...
func (g *Generator) loadNews(ctx context.Context) ([]ArticleView, []CategoryView, error) {
	...
	for _, column := range columns {
		...
		for index, item := range items {
			view := makeView(g.cfg, column.Name, item)
...
func (g *Generator) renderArticle(root string, column model.Column, article model.Article) (string, error) {
	view := makeView(g.cfg, column.Name, article)
	...
	view.Content = template.HTML(normalizeContentMediaURLs(g.sanitizer.Sanitize(article.Content)))
...
func (g *Generator) writeGeneratedContent(ctx context.Context, root string) error {
	...
	for _, article := range important {
		...
		view := makeView(g.cfg, category, article)
	...
	for _, parent := range parents {
		...
		for _, child := range children {
			...
			article := items[0]
			view := makeView(g.cfg, parent.Name, article)
			payload.ServiceItems[child.Code] = serviceItem{ArticleID: article.ID, Category: parent.Name, Title: article.Title, Lead: article.Summary, Image: view.Cover, ImageAlt: article.Title, Content: normalizeContentMediaURLs(g.sanitizer.Sanitize(article.Content))}
		}
	}
	loadAbout := func(columnName string) ([]aboutItem, error) {
		...
		for _, article := range items {
			view := makeView(g.cfg, columnName, article)
			...
			if href == "" {
				...
				if len(attachments) > 0 {
					href = mediaURL(g.cfg, attachments[0].URL)
