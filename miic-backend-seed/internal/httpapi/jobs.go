...
type Job struct {
	...
	Progress   generator.Progress `json:"progress"`
...
func (m *jobManager) start(kind string, run func(context.Context) (any, error)) (Job, error) {
	...
	job := &Job{
		...
		Progress: generator.Progress{Stage: "等待执行"}, CreatedAt: now, UpdatedAt: now,
	}
...
func (m *jobManager) run(id string, jobCtx context.Context, control *jobControl, run func(context.Context) (any, error)) {
	...
	ctx := generator.WithProgress(jobCtx, func(progress generator.Progress) {
...
func completedGeneratedFiles(result any) (int, bool) {
	...
	case generator.GenerationResult:
...
func (m *jobManager) updateProgress(id string, progress generator.Progress) {
	m.mu.Lock()
	defer m.mu.Unlock()
